...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        time.sleep(2)  # zajistí, že session je aktivní před publikací
...
def fetch_host_stats(xo_url, host_uuid, token, verify_ssl=True):
    ...
    try:
        ...
        net_tx_kbps_series = [round(v / 1000, 2) for v in tx_metrics[-NUM_SAMPLES:]]
        net_rx_kbps_series = [round(v / 1000, 2) for v in rx_metrics[-NUM_SAMPLES:]]
